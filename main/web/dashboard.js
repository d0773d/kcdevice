...
async function startHttpFocusFallback(target){try{const resp=await fetch('/api/sensors/pause',{method:'POST'});if(resp.ok){focusPauseIssued=true;}}catch(err){console.warn('HTTP pause failed',err);}await pollFocusedSensorHttp();focusSensorTimer=setInterval(pollFocusedSensorHttp,2500);}
